...
EditSelectFactory::EditSelectFactory()
{
	...
	editSelectDeformationVertices = new QAction("Select vertices for deformation", this);

    actionList << editSelectVert;
    actionList << editSelect;
    actionList << editSelectConnected;

	actionList << editSelectDeformationVertices;
...
MeshEditInterface* EditSelectFactory::getMeshEditInterface(QAction *action)
{
  ...
  else if (action == editSelectDeformationVertices) {
	  return new EditSelectPlugin(EditSelectPlugin::SELECT_DEFORMATION_VERT_MODE);
  }

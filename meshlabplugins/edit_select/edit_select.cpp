...
EditSelectPlugin::EditSelectPlugin(int ConnectedMode) :selectionMode(ConnectedMode), deformationDialog(nullptr){
	isDragging = false;
}
...
void EditSelectPlugin::Decorate(MeshModel &m, GLArea * gla)
{
	if (isDragging)
	{
		...
		else if (selectionMode == SELECT_DEFORMATION_VERT_MODE)
		{
			if (selectFrontFlag) {
		...
		else if (selectionMode == SELECT_FACE_MODE)
		{
			//m.cm.selface.clear();
...
bool EditSelectPlugin::StartEdit(MeshModel & m, GLArea * gla, MLSceneGLSharedDataContext* /*cont*/)
{
	...
	if (selectionMode == SELECT_DEFORMATION_VERT_MODE) {
		if (deformationDialog == nullptr) {
			deformationDialog = new QDialog(gla);
			deformationDialogUI.setupUi(deformationDialog);

			m.updateDataMask(MeshModel::MM_FACEFACETOPO);
			m.updateDataMask(MeshModel::MM_VERTFACETOPO);

			tri::UpdateColor<CMeshO>::PerVertexConstant(m.cm, vcg::Color4b::White);
			tri::UpdateTopology<CMeshO>::FaceFace(m.cm);
			tri::UpdateTopology<CMeshO>::VertexFace(m.cm);

			connect(deformationDialogUI.pushButton_confirmSelection, &QPushButton::clicked, this, [&, gla]() {
				vcg::Color4b vColor = deformationDialogUI.radioButton_handled->isChecked() ? vcg::Color4b::Red : vcg::Color4b::Blue;
				tri::UpdateSelection<CMeshO>::VertexFromFaceLoose(m.cm);
				tri::UpdateColor<CMeshO>::PerVertexConstant(m.cm,vColor, true);
				tri::UpdateSelection<CMeshO>::Clear(m.cm);
				gla->update();
			});

			connect(deformationDialogUI.pushButton_output, &QPushButton::clicked, this, [&]() {
				QString filePath = QFileDialog::getExistingDirectory(nullptr, "请选择导出路径...", "", QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);

				QFile outFile(filePath + '/' + "bone.dmat");
				QTextStream exput(&outFile);
				if (outFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
					exput << 1 << " " << m.cm.vert.size() << '\n';
					for (CMeshO::VertexIterator vi = m.cm.vert.begin(); vi != m.cm.vert.end(); ++vi) {
						if (vi->C() == vcg::Color4b(vcg::Color4b::Red)) { //1
							exput << 1 <<'\n';
						}
						else if (vi->C() == vcg::Color4b(vcg::Color4b::Blue)) { //0
							exput << 0 << '\n';
						}
						else {
							exput << -1 << '\n';

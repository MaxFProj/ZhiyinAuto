...
public:
    ...
    */
	...
	std::vector<vcg::Matrix44f> moveMatrix;
	std::vector<vcg::Matrix44f> move_matrix_without_r;
	vcg::Point3f bary_center;
	vcg::Point3f init_pca[3];
    vcg::KdTree<float>* m_kdtree = nullptr;

    AABBTree* m_AABBTree = nullptr;
    geodesic::Mesh mesh_geodesic;
    std::vector<vcg::Point3f> pointsForLaser;	// 用于激光切割的两个位置点（先左 再右）
    std::vector<vcg::Point3f> ctrlPoints;    // 切割曲线控制点
    std::vector<vcg::Point3f> curve;		 // 有控制点拟合成的切割曲线
    std::vector<vcg::Point3f> curveDir;      // 控制切割方向
    std::vector<vcg::Point3f> labelDir;      // 切割线标志物的正面朝向

    CMeshO* label_mesh      = nullptr;
    CMeshO* qr_mesh         = nullptr;
    CMeshO* base_mesh       = nullptr;
    CMeshO* info_mesh       = nullptr;
    CMeshO* intersected_mesh = nullptr;

    std::vector<vcg::Matrix44f> eruption_rotation_mat;  //萌出帽模型从世界坐标系变换至局部坐标系的旋转矩阵
    std::vector<vcg::Matrix44f> eruption_trans_mat;
    std::vector<vcg::Matrix44f> eruption_scale_mat;
    std::vector<vcg::Matrix44f> eruption_mat;            // 通过控制轴改变萌出帽位置后的最终变换矩阵
...
signals:
    ...
    // By SongLei:
    //tell mainwindow to append dw to its left
    void addNewDockWidget(QDockWidget*, bool);

 public:
    // emit signal to add new dockwidget to main window
    void emitAddNewDockWidget(QDockWidget* dw, bool needAdd = true);

...
void MeshDocument::refreshMeshList() {
    emit meshSetChanged();
}

void MeshDocument::resetTrackballFunc()
{
...
void MeshDocument::emitAddNewDockWidget(QDockWidget* dw, bool needAdd) {
    emit addNewDockWidget(dw, needAdd);
}
...
MeshModel::~MeshModel()
{
    ...
    if(m_AABBTree)  delete m_AABBTree;
    if(label_mesh)  delete label_mesh;
    if(qr_mesh)     delete qr_mesh;
    if(base_mesh)   delete base_mesh;
    if(info_mesh)   delete info_mesh;
    if (intersected_mesh) delete intersected_mesh;

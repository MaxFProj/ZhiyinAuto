...
void MainWindow::newProject(const QString& projName)
{
    ...
    QMdiSubWindow* sub = mdiarea->addSubWindow(mvcont);
    ...
    //connect(&(mvcont->meshDoc), SIGNAL(meshSetChanged()), this, SLOT(updateLayerDialog()), Qt::QueuedConnection);
    connect(&(mvcont->meshDoc), &MeshDocument::resetTrackball, this, [&] (){
            std::cout << "refresh rendering data and reset track ball\n";
            MLSceneGLSharedDataContext* shared = _currviewcontainer->sharedDataContext();
            if (shared != NULL) {
                for (MeshModel* current_model: _currviewcontainer->meshDoc.meshList) 
                {
                    MLRenderingData curr;
                    getRenderingData(current_model->id(), curr);

                    // update all mesh attributes
                    MLRenderingData::RendAtts dttoupdate;
                    MLPoliciesStandAloneFunctions::fromMeshModelMaskToMLRenderingAtts(MeshModel::MM_ALL, dttoupdate);
                    shared->meshAttributesUpdated(current_model->id(), true, dttoupdate);

                    setRenderingData(current_model->id(), curr);
                }
            }
            GLA()->resetTrackBall();
     });

    connect(&(mvcont->meshDoc), &MeshDocument::addNewDockWidget, this, [&](QDockWidget* dw, bool needAdd) {
        if (dw == nullptr) {
            return;
        }
        else {
            if (needAdd) {
                tabifyDockWidget(layerDialog, dw);
                dw->show();
                dw->raise();
            }
            else {
...
void MainWindow::changeImportFileNames()
{
    ...
    if (filename_ui == nullptr) {
        ...
        connect(filename_dialog, &QDialog::accepted, this, [&]() {
            // rename mesh
            for (int row_count = 0; row_count < meshDoc()->meshList.size(); ++row_count) {
                ...
                QString patientID = filename_ui->lineEdit_patientID->text();
                meshDoc()->meshList.at(row_count)->setLabel(patientID + '&' + jaw + '&' + step);
                qDebug() << meshDoc()->meshList.at(row_count)->label();
            }
        });
    }
    ...
    //fill id textline
    QString patientID = meshDoc()->meshList.front()->fullName();
    patientID = patientID.split('/').at(patientID.split('/').size() - 2);
    filename_ui->lineEdit_patientID->setText(patientID);
    ...
    filename_ui->tableWidget->setRowCount(meshDoc()->meshList.size());

    for (int row_count = 0; row_count < meshDoc()->meshList.size(); ++row_count) {
        MeshModel* m = meshDoc()->meshList.at(row_count);
        QString label = m->label().mid(0, m->label().lastIndexOf('.'));

        // match step
        QRegularExpressionMatchIterator step_match_ite = step_re.globalMatch(label);
        ...
        //match jaw
        QRegularExpressionMatch match_upper = upper_re.match(label);
        QRegularExpressionMatch match_lower = lower_re.match(label);
        ...
        filename_ui->tableWidget->setItem(row_count, 0, new QTableWidgetItem(m->label()));
...
bool MainWindow::importMesh(QString fileName,bool isareload)
{
    ...
    QStringList fileNameList;
    if (fileName.isEmpty())
        fileNameList = QFileDialog::getOpenFileNames(this, tr("Import Teeth Mesh"), lastUsedDirectory.path(), PM.inpFilters.join(";;"));
    else
...
void MainWindow::showTrackBall() {if(GLA() != 0) 	GLA()->showTrackBall(false);}

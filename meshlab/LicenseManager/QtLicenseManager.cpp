...
LicenseCheckerThread* LicenseCheckerThread::m_pInstance = nullptr;
...
void LicenseCheckerThread::setMessageHandler( void* pHandler ) const
{
	connect(this, SIGNAL(message(int, const QString &, const QString &, int*, QMessageBox::StandardButtons, QMessageBox::StandardButton)),
		static_cast<emxMessageHandler*>(pHandler), SLOT(messageBox(int, const QString &, const QString &, int*, QMessageBox::StandardButtons, QMessageBox::StandardButton)));
}
...
{
	m_registerDialog = new emxRegisterDialog;
 ...
 */
 ...
 {	
	...
	printf("RegisterMode %d", RegisterMode);
...
// 		{
// 			sprintf(msg, "%s", QApplication::translate("QtLicenseManager", "Error[%1]:The time setting of this computer is abnormal. Please correct it before trying to start emX again, or it'll be locked forever.", 0).arg(status).toLocal8Bit().data());
// 		}
// 		else
// 		{

...
bool emxCodeRegister::RegisterAndValidate()
{
	QSettings settings(QSettings::NativeFormat, QSettings::UserScope, ORGNIZATION, APPLICATION);
	// 用于存储已经用过的注册码的文件
	...
	if(regFile == nullptr)
	{
		...
		int ret;
//		QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("Error opening the data file, please check the whether the file and the directory has read and write permission,or contact Hangzhou Meiqi Technology."));
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Error opening the data file, please check the whether the file and the directory has read and write permission."),&ret);
		//fclose(regFile);
		return false;
	}
	...
	if(!GenerateVolumeInfo(RegID, TrialRegID, LimitedRegID, FullRegID)) {
		...
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Error reading the register information, please contact Hangzhou Meiqi Technology."),&ret);
		return false;
	}
	// 注册码
...
//							int ret;
//							m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("invalidate register code,please reenter register code again or contact Hangzhou Meiqi Technology"),&ret);
//							fclose(regFile);
//							return false;
//						}
//					}
...
//				break;
		...
		if (TrialRegID == QString(regCodeGenID)) { 	//注册码与硬盘地址匹配
			...
			int ret;
			m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("invalidate register code,please reenter register code again"),&ret);
		}
	} while(!isValid);
	//////////////////////////////////////////////////////////////////////////
	...
	if(isValid) {
		...
		if(!isRegisted && !isCodeGenTimeValid(regCodeGenTime))
		{
			...
			int ret;
			m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Register code is out of date."),&ret);
		...
		if(regType == 1 || regType == 2)
		{
			...
			if(settings.value("RegCodeBack", -1).toInt() == -1) // 注册时间不存在
			{
				if(isRegisted) // 已注册，但注册时间已被删除
				{
					...
					int ret;
					m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("The software license is out of date"),&ret);
					// 删除注册码
					if(settings.value("RegCode", -1).toInt() != -1)
						settings.remove("RegCode");
					return false;//FullRegisterAndValidate(processReg, timeReg, RegID, LimitedRegID, FullRegID);
			...
			else // 注册时间已存在
			{
				...
				if(!IsValidTime(RegTime))
				{
					//AfxMessageBox("emX软件已过使用期，续订请联系浙江大学CAD&CG国家重点实验室");
					int ret;
					m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Software is out of date"),&ret);
					//删除注册码和注册时间
					if(settings.value("RegCode",-1).toInt() != -1)
						settings.remove("RegCode");
...
emxCodeRegister::emxCodeRegister( emxMessageHandler * handle, emxRegisterDialog* dialog)
{
	regFile = fopen("data.db", "ab+");
	if(regFile == NULL)
	{
		//AfxMessageBox("emX软件数据文件打开错误，请检查程序目录及文件是否具有读写权限，或联系浙江大学CAD&CG国家重点实验室");
		int ret;
		//QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact Hangzhou Meiqi Technology"));
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("iMQ open the data file error,please check the whether the file and the directory has read and write permission,or contact Hangzhou Meiqi Technology"),&ret);
		fclose(regFile);
		exit(0);
	}
	m_handle = handle;
...
bool emxCodeRegister::GenerateVolumeInfo(QString& RegID, QString& TrialRegID, QString& LimitedRegID, QString& FullRegID)
{
#ifdef Q_OS_WIN32
	QStringList envs = QProcess::systemEnvironment();
	QString sysroot;
	for (QString str:envs)
	{
		if (str.startsWith("windir="))
		{
			sysroot = str.right(str.length() - 7);
			break;
		}
	}
	qDebug() << sysroot;
	...
	if (!bget)
		return false;
	qDebug() << "Volume SN:" << VolumeSerialNumber;
#endif
	// 转换硬盘序列号为十六进制字符
...
char emxCodeRegister::SumToChar( unsigned char a, unsigned char b )
{
...
void emxCodeRegister::RegIDToTrial(char regID[21], char trialRegID[21])
{
	for(int i = 0; i < 20; ++i)
...
void emxCodeRegister::RegIDToLimited(char regID[21], char limitedRegID[21])
{
	for(int i = 0; i < 10; ++i) {
...
void emxCodeRegister::RegIDToFull(char regID[21], char fullRegID[21])
{
	for(int i = 0; i < 10; ++i) {
...
void emxCodeRegister::GetCurrentRegTime( char regTime[255] )
{
...
bool emxCodeRegister::DecodeRegCode( char regCode[255], char codeGenID[255], time_t& codeGenTime )
{
	// 判断注册码与注册标识符是否匹配
...
unsigned char emxCodeRegister::CharToHex( unsigned char ch )
{
	//convert	A----P to 0---15
	if ( ch >= 'A' && ch <= ('A'+15))
		return (ch - 'A');		
...
struct tm emxCodeRegister::regTimeStr2TM( char regTime[REG_TIME_LENGTH] )
{
...
bool emxCodeRegister::isCodeGenTimeValid( time_t regTime )
{
	time_t currentTime = time(0);
	int keyUseLimit = 10;			// 限制注册码生成后在10天之内使用有效
	time_t keyDeadLine = regTime + keyUseLimit * 24 * 3600;
	printf("keyDeadLine:%lld\n", keyDeadLine);
	printf("currentTime:%lld\n", currentTime);
	printf("registeTime:%lld\n", regTime);
	printf("buildTimeSt:%lld\n", buildTimestamp());
	// 注册码在编译时间之后生成 && 注册码生成时间小于当前时间 && 当前时间在注册码生成后10天之内
	if (buildTimestamp() <= regTime && regTime <= currentTime && currentTime <= keyDeadLine)
		return true;
	else
		return false;
}

void emxCodeRegister::getCurrentDateTime( char regTime[255] )
{
	time_t temptime= time(0);
	tm *temptm = localtime(&temptime);
	if(temptm->tm_year <  113)
...
bool emxCodeRegister::IsValidTime( QString& RegTime )
{
	...
	if (regTime >= buildTimestamp() && currentTime >= buildTimestamp() && currentTime <= endTime)
		return true;
	else
		return false;
}

time_t emxCodeRegister::getLicenseEndTime( time_t regTime )
{
	time_t endTime = 0;
	if(regType == 1)
	{
	...
	else if (regType == 2)
	{
		//////////////////////////////////////////////////////////////////////////
		// 按天数试用
		int licenseValidDayCount = 30;//证书有效天数
		endTime = regTime + licenseValidDayCount * 24 * 3600;
	}
	return endTime;
}

bool emxCodeRegister::timeCheatCheck( QSettings &setting )
{
	bool ret = false;
	char regTime[255];
	ULONG regNum = 255;
	...
	if(setting.value("RegCodeBackBk",-1).toInt() == -1)
	{
		...
		int re;
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("check iMQ register information error,please reenter the register code or contact Hangzhou Meiqi Technology(Error Code:TC001)"),&re);
		ret = false;
	}
	else
	{
...
time_t emxCodeRegister::decodeCurrentDateTime( char regTime[255] )
{
	//用于验证前面4个字节的信息
	tm temptm;
...
bool emxCodeRegister::isValidRunTime( time_t t )
{
	QSettings settings(QSettings::NativeFormat, QSettings::UserScope, ORGNIZATION, APPLICATION);
	if(settings.value("RegCodeBack", -1).toInt() != -1)
	{
		...
		QByteArray tregTime = settings.value("RegCodeBack", -1).toByteArray();
		tregTime.append("\0");
		strcpy(regTimeBuf, tregTime);
		tm regTime_tm = regTimeStr2TM(regTimeBuf);
		time_t regTime = mktime(&regTime_tm);
		time_t endTime = getLicenseEndTime(regTime);
		if (t >= buildTimestamp() && t >= regTime && t <= endTime)
		{
			return true;
		}
	}
...
time_t emxCodeRegister::buildTimestamp()
{
	...
	struct tm buildTime_tm;
	memset(&buildTime_tm, 0, sizeof(struct tm));
	buildTime_tm.tm_year = iYear - 1900;
	buildTime_tm.tm_mon = iMonth;
	buildTime_tm.tm_mday = iDay;


// 	sprintf(szTmpTime, "%s", __TIME__);    //"10:59:19"
// 	sscanf(szTmpTime, "%d:%d:%d", &iHour, &iMin, &iSec);
// 	buildTime_tm.tm_hour = iHour;
// 	buildTime_tm.tm_min = iMin;
// 	buildTime_tm.tm_sec = iSec;

	time_t t = mktime(&buildTime_tm);
//	printf("Build Time : %lld -> %s \n", t, ctime(&t));
	return t;
}
/*
bool emxCodeRegister::FullRegisterAndValidate(QSettings& setting, QString& RegID, QString& TrialRegID, QString& LimitedRegID, QString& FullRegID)
{
	// 注册
	m_registerDialog->setRegID(RegID);
	...
	if(regType == 1 && LimitedRegID == QString(regCodeGenID)) {
		...
		if (!isCodeGenTimeValid(regCodeGenTime))
		{
			...
			int ret;
			m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("register code is out of data,please contact Hangzhou Meiqi Technology"),&ret);
			...
			m_handle->messageBox(QMessageBox::Information, tr("Register Success"),tr("Upgrade successfully"),&ret);
		...
		int ret;
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("invalidate register code,please contact Hangzhou Meiqi Technology"),&ret);
...
bool emxCodeRegister::FullRegisterAndValidate( QSettings& setting, QString& RegID, QString& LimitedRegID, QString& FullRegID )
{
	// 注册
	// 注册
...
*/
bool emxCodeRegister::FullRegisterAndValidate( QString& RegID, QString& TrialRegID,QString& LimitedRegID, QString& FullRegID )
{
	QSettings setting(QSettings::NativeFormat, QSettings::UserScope, ORGNIZATION, APPLICATION);
	// 注册码
	char regCode[255], regTime[255], regCodeGenID[255];
	ULONG charNum = 255, regNum = 255;
...
void emxCodeRegister::setRegisterDialog(emxRegisterDialog * dialog)
{
	m_registerDialog = dialog;
}

QString emxCodeRegister::GetRegID(QString& RegID, QString& TrialRegID, QString& LimitedRegID, QString& FullRegID)
{
	// 用于存储已经用过的注册码的文件
	// 所有用户输入的有效注册码均记录在此文件中，防止一个注册码被多次使用
	//fclose(regFile);
	...
	//	//AfxMessageBox("emX软件数据文件打开错误，请检查程序目录及文件是否具有读写权限，或联系浙江大学CAD&CG国家重点实验室");
		//int ret;
	//	//QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact Hangzhou Meiqi Technology"));
		//m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact Hangzhou Meiqi Technology"),&ret);
	//	exit(0);
	//}
	if(!GenerateVolumeInfo(RegID, TrialRegID, LimitedRegID, FullRegID)) {
		// 无法获取本地C盘序列号
		int ret;
		//AfxMessageBox("读取注册信息失败，请联系浙江大学CAD&CG国家重点实验室");
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("iMQ read the register information error,please contact Hangzhou Meiqi Technology"),&ret);
		exit(0);
	}
	return RegID;
}

emxCodeRegister::~emxCodeRegister()
{
	fclose(regFile);
}

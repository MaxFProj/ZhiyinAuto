...
//	setPage(Page_SentinelCheck,new sentinelCheckPage);
	...
	//	QMessageBox::critical(this, tr("Register Error"),tr("emX read the register information error,please contact your software provider"));
	//	exit(0);
	//}
...
QString emxRegisterDialog::getRegID()
{
	if (m_RegID.length()==0)
	{
		if(!m_codeRegister->GenerateVolumeInfo(m_RegID, m_TrialRegID, m_LimitedRegID, m_FullRegID)) {
			// 无法获取本地C盘序列号
			//AfxMessageBox("读取注册信息失败，请联系浙江大学CAD&CG国家重点实验室");
			QMessageBox::critical(this, tr("Register Error"),tr("imQ read the register information error,please contact your software provider"));
			exit(0);
		}
	}
	return m_RegID;
...
// 	//setSubTitle(tr("Please enter the registration code below.If you haven't registration code,please contact your software provider."));
// 	setSubTitle(tr("Please enter the Registration code below."));
	setTitle(tr("Registration"));
	setSubTitle(tr("This distribution of the software is a trial version which is only used for technique preview. Please enter the registration code below to continue."));
...
IntroPage::IntroPage( QWizard *parent /*=0*/ )
{
	...
	setSubTitle(tr("This distribution of the software is a trial version which is only used for technique preview."));
	verticalLayout = new QVBoxLayout();
	label = new QLabel("");
	//label = new QLabel(("emx is a high frequency electromagnetic scattering simulation system designed by your software provider."));
	QFont ft;
	ft.setPointSize(14);
	label->setFont(ft);
	verticalLayout->addWidget(label);

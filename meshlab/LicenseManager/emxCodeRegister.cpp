...
bool emxCodeRegister::RegisterAndValidate()
{
	...
	if(regFile == nullptr)
	{
		...
		int ret;
//		QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("Error opening the data file, please check the whether the file and the directory has read and write permission,or contact your software provider."));
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Error opening the data file, please check the whether the file and the directory has read and write permission,or contact your software provider."),&ret);
		//fclose(regFile);
		return false;
	}
	...
	if(!GenerateVolumeInfo(RegID, TrialRegID, LimitedRegID, FullRegID)) {
		...
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Error reading the register information, ."),&ret);
		return false;
	}
	// 注册码
...
//							int ret;
//							m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("invalidate register code,please reenter register code again or contact your software provider"),&ret);
//							fclose(regFile);
//							return false;
//						}
//					}
...
emxCodeRegister::emxCodeRegister( emxMessageHandler * handle, emxRegisterDialog* dialog)
{
	...
	if(regFile == NULL)
	{
		//AfxMessageBox("emX软件数据文件打开错误，请检查程序目录及文件是否具有读写权限，或联系浙江大学CAD&CG国家重点实验室");
		int ret;
		//QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact your software provider"));
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("Open the data file error,please check the whether the file and the directory has read and write permission,or contact your software provider"),&ret);
		fclose(regFile);
		exit(0);
	}
	m_handle = handle;
...
bool emxCodeRegister::isCodeGenTimeValid( time_t regTime )
{
	...
	double keyUseLimit = 10;			// 限制注册码生成后在10天之内使用有效
...
time_t emxCodeRegister::getLicenseEndTime( time_t regTime )
{
	...
	else if (regType == 2)
	{
		//////////////////////////////////////////////////////////////////////////
		// 按天数试用
		double licenseValidDayCount = 10;//证书有效天数
...
bool emxCodeRegister::timeCheatCheck( QSettings &setting )
{
	...
	if(setting.value("RegCodeBackBk",-1).toInt() == -1)
	{
		...
		int re;
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("check register information error,please reenter the register code(Error Code:TC001)"),&re);
		ret = false;
	}
	else
	{
...
bool emxCodeRegister::FullRegisterAndValidate(QSettings& setting, QString& RegID, QString& TrialRegID, QString& LimitedRegID, QString& FullRegID)
{
	...
	if(regType == 1 && LimitedRegID == QString(regCodeGenID)) {
		...
		if (!isCodeGenTimeValid(regCodeGenTime))
		{
			...
			int ret;
			m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("register code is out of data,"),&ret);
		...
		int ret;
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("invalidate register code,"),&ret);
...
QString emxCodeRegister::GetRegID(QString& RegID, QString& TrialRegID, QString& LimitedRegID, QString& FullRegID)
{
	...
	//	//AfxMessageBox("emX软件数据文件打开错误，请检查程序目录及文件是否具有读写权限，或联系浙江大学CAD&CG国家重点实验室");
		//int ret;
	//	//QMessageBox::warning((QWidget*)(this->parent()),tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact your software provider"));
		//m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("emX open the data file error,please check the whether the file and the directory has read and write permission,or contact your software provider"),&ret);
	//	exit(0);
	//}
	if(!GenerateVolumeInfo(RegID, TrialRegID, LimitedRegID, FullRegID)) {
		// 无法获取本地C盘序列号
		int ret;
		//AfxMessageBox("读取注册信息失败，请联系浙江大学CAD&CG国家重点实验室");
		m_handle->messageBox(QMessageBox::Critical, tr("Register Error"),tr("read the register information error,"),&ret);
		exit(0);
	}
	return RegID;
}
